...
py::array_t<int> sample_negative_ByPos(int user_num, int item_num, int train_num, std::vector<std::vector<int>> allPos, int neg_num)
{
    int row = neg_num + 2;
    ...
    {
        std::vector<int> pos_item = allPos[user];
        for (int pair_i = 0; pair_i < pos_item.size(); pair_i++)
        {
            int negitem = 0;
            ptr[(index + pair_i) * row] = user;
            ptr[(index+ pair_i) * row + 1] = pos_item[pair_i];
            for (int neg_index = 2; neg_index < neg_num + 2; neg_index++)
            {
                do
                {
                    negitem = randint_(item_num);
                    ...
                    find(pos_item.begin(), pos_item.end(), negitem) != pos_item.end());
                ptr[(index + pair_i) * row + neg_index] = negitem;
            }
        }
        index = index + pos_item.size();
    }
    return S_array;
}
...
PYBIND11_MODULE(sampling, m)
{
    ...
    m.def("sample_negative_ByPos", &sample_negative_ByPos, "sampling negatives for all positive items",
          "user_num"_a, "item_num"_a, "train_num"_a, "allPos"_a, "neg_num"_a);
